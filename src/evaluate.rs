...
const GAME_PHASE: [i32; 6] = [0, 1, 1, 2, 4, 0];
...
impl Evaluator {
    pub fn new() -> Self {
        let mut mg_table = [[0; 64]; 6];
        let mut eg_table = [[0; 64]; 6];
        for pc in Piece::iter() {
            for sq in 0..64 {
                mg_table[pc as usize][sq as usize] = MG_VALUE[pc as usize] + MG_TABLES[pc as usize][sq as usize];
                eg_table[pc as usize][sq as usize] = EG_VALUE[pc as usize] + EG_TABLES[pc as usize][sq as usize];
            }
        }
        Evaluator {
            mg_table,
    ...
    pub fn evaluate(&self, board: &Board) -> i32 {
        ...
        if board.is_white() { score } else { -score }
