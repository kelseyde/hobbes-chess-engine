...
impl Histories {
    ...
    pub fn capture_history_score(
        ...
        self.capture_history
            .get(board.stm, *mv, pc, captured, threats) as i32
    }
...
impl QuietHistory {
    ...
    pub fn update(&mut self, stm: Side, mv: &Move, threats: Bitboard, bonus: i16) {
        ...
        *entry = gravity(*entry, bonus as i32, Self::MAX);
    }
...
impl CaptureHistory {
    ...
    pub fn update(
        ...
        let threat_index = ThreatIndex::new(mv, threats);
        let entry =
            &mut self.entries[stm][threat_index.from()][threat_index.to()][pc][mv.to()][captured];
        *entry = gravity(*entry, bonus as i32, Self::MAX as i32);
    }
...
fn gravity(current: i16, update: i32, max: i32) -> i16 {
    let update = update.clamp(-max, max);
    current + (update - update.abs() * current as i32 / max) as i16
}
