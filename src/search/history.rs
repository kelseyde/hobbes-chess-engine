...
impl Histories {
    ...
    pub fn update_continuation_history(
        ...
        for prev_ply in ContinuationHistory::PLIES {
            if ply >= prev_ply && (prev_ply <= 2 || !in_check) {
                let prev_mv = ss[ply - prev_ply].mv;
                let prev_pc = ss[ply - prev_ply].pc;
...
pub fn cont_history_4_bonus(depth: i32) -> i16 {
    let scale = cont_hist_4_bonus_scale() as i16;
    let offset = cont_hist_4_bonus_offset() as i16;
    let max = cont_hist_4_bonus_max() as i16;
    history_bonus(depth, scale, offset, max)
}

pub fn cont_history_6_bonus(depth: i32) -> i16 {
    let scale = cont_hist_6_bonus_scale() as i16;
    let offset = cont_hist_6_bonus_offset() as i16;
    let max = cont_hist_6_bonus_max() as i16;
    history_bonus(depth, scale, offset, max)
}
...
pub fn cont_history_4_malus(depth: i32) -> i16 {
    let scale = cont_hist_4_malus_scale() as i16;
    let offset = cont_hist_4_malus_offset() as i16;
    let max = cont_hist_4_malus_max() as i16;
    history_malus(depth, scale, offset, max)
}

pub fn cont_history_6_malus(depth: i32) -> i16 {
    let scale = cont_hist_6_malus_scale() as i16;
    let offset = cont_hist_6_malus_offset() as i16;
    let max = cont_hist_6_malus_max() as i16;
    history_malus(depth, scale, offset, max)
}
...
pub fn lmr_conthist_4_bonus(depth: i32, good: bool) -> i16 {
    if good {
        let scale = lmr_cont_hist_4_bonus_scale() as i16;
        let offset = lmr_cont_hist_4_bonus_offset() as i16;
        let max = lmr_cont_hist_4_bonus_max() as i16;
        history_bonus(depth, scale, offset, max)
    } else {
        let scale = lmr_cont_hist_4_malus_scale() as i16;
        let offset = lmr_cont_hist_4_malus_offset() as i16;
        let max = lmr_cont_hist_4_malus_max() as i16;
        history_malus(depth, scale, offset, max)
    }
}

pub fn lmr_conthist_6_bonus(depth: i32, good: bool) -> i16 {
    if good {
        let scale = lmr_cont_hist_6_bonus_scale() as i16;
        let offset = lmr_cont_hist_6_bonus_offset() as i16;
        let max = lmr_cont_hist_6_bonus_max() as i16;
        history_bonus(depth, scale, offset, max)
    } else {
        let scale = lmr_cont_hist_6_malus_scale() as i16;
        let offset = lmr_cont_hist_6_malus_offset() as i16;
        let max = lmr_cont_hist_6_malus_max() as i16;
        history_malus(depth, scale, offset, max)
    }
}
