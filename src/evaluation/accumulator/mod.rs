...
impl BoardObserver for Accumulator {
    ...
    fn on_piece_transform(&mut self, board: &Board, old_pc: Piece, old_side: Side, new_pc: Piece, new_side: Side, sq: Square) {
        self.psq.subs.push(PieceSquareFeature::new(old_pc, sq, old_side));
        self.psq.adds.push(PieceSquareFeature::new(new_pc, sq, new_side));
        self.threat.push_piece_transform(board, old_pc, old_side, new_pc, new_side, sq);
    }
}