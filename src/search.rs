...
fn alpha_beta(board: &Board,
    ...
    if !in_check {
        ...
        complexity = td.correction_history.squared_correction_terms(board, &td.ss, ply);
    ...
    while let Some(mv) = move_picker.next(board, td) {
        ...
        if depth >= lmr_min_depth() && searched_moves > lmr_min_moves() + root_node as i32 + 2 * pv_node as i32 {
            ...
            let complexity_r = complexity / 8192;
            td.count += 1;
            if complexity_r > td.max {
                td.max = complexity_r;
            }
            if complexity_r < td.min {
                td.min = complexity_r;
            }
            td.total += complexity_r;
            td.avg = td.total / td.count;
            if td.nodes % 2000 == 0 {
                println!("complexity_r: {} min: {} max: {} avg: {}", complexity_r, td.min, td.max, td.avg);
            }
            r -= complexity / 3;
