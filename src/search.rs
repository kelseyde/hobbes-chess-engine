    ...
    while let Some(mv) = move_picker.next(board, td) {
        ...
        if !root_node
            ...
            if score < s_beta {
                ...
                let double_ext_margin = (se_dext_margin(is_quiet) - (correction.abs() / 12)).max(0);
                extension += (!pv_node && score < s_beta - double_ext_margin) as i32;

                let triple_ext_margin = se_text_margin(is_quiet);
        ...
        if depth >= lmr_min_depth() && searched_moves > lmr_min_moves() + root_node as i32 + 2 * pv_node as i32 {
            ...
            let reduced_depth = (new_depth - (r / 1024)).clamp(1, new_depth + (1 + (legal_moves <= 3) as i32));
