...
fn alpha_beta<NODE: NodeType>(
    ...
    if best_move.exists() {
        ...
        let eval_fail_low = (!in_check && static_eval < alpha) as i16;
        ...
        let quiet_bonus = quiet_history_bonus(depth)
            ...
            + eval_fail_low as i16 * quiet_hist_fail_low_bonus() as i16;

        let quiet_malus = quiet_history_malus(depth)
            + new_tt_move as i16 * quiet_hist_ttmove_malus() as i16
            + eval_fail_low as i16 * quiet_hist_fail_low_malus() as i16;
        ...
        let cont_1_bonus = cont_history_1_bonus(depth)
            ...
            + eval_fail_low as i16 * cont_hist_1_fail_low_bonus() as i16;

        let cont_1_malus = cont_history_1_malus(depth)
            + new_tt_move as i16 * cont_hist_1_ttmove_malus() as i16
            + eval_fail_low as i16 * cont_hist_1_fail_low_malus() as i16;
        ...
        let cont_2_bonus = cont_history_2_bonus(depth)
            ...
            + eval_fail_low as i16 * cont_hist_2_fail_low_bonus() as i16;

        let cont_2_malus = cont_history_2_malus(depth)
            + new_tt_move as i16 * cont_hist_2_ttmove_malus() as i16
            + eval_fail_low as i16 * cont_hist_2_fail_low_malus() as i16;
        ...
        let from_bonus = from_history_bonus(depth)
            + eval_fail_low as i16 * from_hist_fail_low_bonus() as i16;

        let from_malus = from_history_malus(depth)
            + eval_fail_low as i16 * from_hist_fail_low_malus() as i16;

        let to_bonus = to_history_bonus(depth)
            + eval_fail_low as i16 * to_hist_fail_low_bonus() as i16;

        let to_malus = to_history_malus(depth)
            + eval_fail_low as i16 * to_hist_fail_low_malus() as i16;
