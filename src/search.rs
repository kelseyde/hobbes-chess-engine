...
fn alpha_beta<NODE: NodeType>(
    ...
    if !in_check {
        ...
        correction = td.correction_history.correction(board, &td.stack, tt_move, ply);
    ...
    if !in_check
        ...
        tt_move = if best_move.exists() { best_move } else { tt_move };
        td.correction_history.update_correction_history(board, &td.stack, depth, ply, static_eval, best_score, tt_move);
    }
...
fn qs(board: &Board, td: &mut ThreadData, mut alpha: i32, beta: i32, ply: usize) -> i32 {
    ...
    if !in_check {
        ...
        let correction = td.correction_history.correction(board, &td.stack, tt_move, ply);
        static_eval = raw_eval + correction;
    }
