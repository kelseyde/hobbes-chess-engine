...
fn alpha_beta<NODE: NodeType>(
            ...
            if score > alpha && new_depth > reduced_depth {
                ...
                if new_depth > reduced_depth {
                    ...
                    if is_quiet && (score <= alpha || score >= beta) {
                        ...
                        let bonuses = [bonus_1, bonus_2, bonus_4];
                        td.history.update_continuation_history(original_board, &td.stack, ply, &mv, pc, &bonuses);
                    }
                }
            }
        }
    ...
    if best_move.exists() {
        ...
        if let Some(captured) = board.captured(&best_move) {
            ...
            td.history.update_continuation_history(board, &td.stack, ply, &best_move, pc, &[cont_1_bonus, cont_2_bonus, cont_4_bonus]);
            ...
            for mv in quiets.iter() {
                if mv != &best_move {
                    ...
                    td.history.update_continuation_history(board, &td.stack, ply, mv, pc, &[cont_1_malus, cont_2_malus, cont_4_malus]);
