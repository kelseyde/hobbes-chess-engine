...
fn alpha_beta<NODE: NodeType>(
    ...
    if best_move.exists() {
        ...
        let capt_fact_bonus = capture_factoriser_bonus(depth);
        let capt_fact_malus = capture_factoriser_malus(depth);
        ...
        if let Some(captured) = board.captured(&best_move) {
            ...
            td.history.capture_history.update(board.stm, pc, &best_move, captured, capt_bonus, capt_fact_bonus);
        } else {
        ...
        for mv in captures.iter() {
            if mv != &best_move {
                if let Some(captured) = board.captured(mv) {
                    td.history.capture_history.update(board.stm, pc, mv, captured, capt_malus, capt_fact_malus);
                }
            }
        }
    }
